...
class RentalAsset(db.Model):
    ...
    asset_type = db.Column(db.Enum('tanah', 'bangunan', name='asset_type_enum'), nullable=False)
    ...
    sertifikat = db.Column(db.Enum('SHM', 'HGB', 'Lainnya', name='sertifikat_enum'), nullable=False)
    jenis_zona = db.Column(db.Enum('Perumahan', 'Komersial', 'Industri', name='jenis_zona_enum'), nullable=False)
    aksesibilitas = db.Column(db.String(100), default='Baik')
    tingkat_keamanan = db.Column(db.Enum('Tinggi', 'Sedang', 'Rendah', name='tingkat_keamanan_enum'), default='Sedang')
    daya_listrik = db.Column(db.String(50), nullable=True)
    kondisi_properti = db.Column(db.String(50), default='Baik')
    deskripsi = db.Column(db.Text, nullable=True)
    status = db.Column(db.Enum('available', 'rented', 'maintenance', 'reserved', name='rental_asset_status_enum'), default='available')
...
class RentalRequest(db.Model):
    ...
    status = db.Column(db.Enum('pending', 'approved', 'rejected', 'active', 'completed', 'cancelled', name='rental_request_status_enum'), default='pending')
