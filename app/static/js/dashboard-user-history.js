...
async function loadRentalRequests(filters = {}) {
    try {
        ...
        // Fetch data from API
        const apiUrl = `/api/user/rental-applications?${queryParams.toString()}&_t=${Date.now()}`;
        ...
        console.log('Stored rental requests:', rentalRequests.length);
        console.log('Approved requests:', rentalRequests.filter(r => r.status === 'approved'));
...
function updateRentalRequestsUI() {
    ...
    console.log('Approved requests in filtered:', filteredRequests.filter(r => r.status === 'approved'));
...
// Pay for approved rental request
async function payRentalRequest(requestId) {
    console.log('payRentalRequest called with ID:', requestId);
    
    // Find the request in local data
    ...
    // Confirm payment
    const assetName = request.asset?.name || request.asset_name || 'Aset';
    const totalAmount = request.total_harga || request.total_price || 0;
    const confirmPayment = confirm(`Lanjutkan pembayaran untuk ${assetName}?\n\nTotal: Rp ${new Intl.NumberFormat('id-ID').format(totalAmount)}`);
    ...
    try {
        ...
        // Call payment API
        const response = await fetch(`/api/rental-payment/${requestId}`, {
            method: 'POST',
            headers: {
        ...
        if (result.snap_token) {
            snap.pay(result.snap_token, {
                onSuccess: function(result) {
                    ...
                    alert('Pembayaran berhasil! Status sewa Anda akan diperbarui.');
                    loadRentalRequests(); // Reload to show updated status
                },
...
function viewRentalDetails(requestId) {
    ...
    if (!request) {
        ...
        // Try to fetch specific request from API
...
// Fetch rental detail from API if not found in local data
async function fetchRentalDetailFromAPI(requestId) {
    try {
        console.log(`Fetching rental detail for ID ${requestId} from API...`);
        ...
        console.log('Fetched rental detail:', result.data);
...
// Show rental detail modal
function showRentalDetailModal(request) {
    console.log('Showing modal for request:', request);
...
window.debugRefreshData = function() {
    console.log('=== DEBUG REFRESH DATA ===');
    loadRentalRequests({});
};

// Debug function to show current rental requests
window.debugShowData = function() {
    console.log('=== CURRENT RENTAL REQUESTS ===');
    console.log('Total requests:', rentalRequests?.length || 0);
    if (rentalRequests) {
        rentalRequests.forEach(req => {
            console.log(`ID: ${req.id}, Status: ${req.status}, Asset: ${req.asset_name || 'N/A'}`);
        });
        console.log('Approved requests:', rentalRequests.filter(r => r.status === 'approved'));
    }
};