...
// Fix for icon scaling issue - disable all animations on page load
document.addEventListener('DOMContentLoaded', function() {
    // Add page-loaded class after a short delay to re-enable transitions
    setTimeout(() => {
        document.body.classList.add('page-loaded');
    }, 500);
});

// Remove dummy data initialization - use only real API data
// All dummy data removed to prevent conflicts with real API data
...
menuLinks.forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    if (targetId === 'sewakan_aset') {
      // Load available assets when navigating to the Sewakan Aset section
      setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
  // Re-initialize dashboard data after DOM is fully loaded to ensure all elements exist
...
function generateChartData(dataSource, groupBy, metric) {
  ...
  // Fetch data dynamically from the updated dataset
  fetch('/api/getChartData', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ dataSource, groupBy, metric })
  })
...
// Dashboard Statistics Loading Function
  ...
  fetch('/api/dashboard/stats')
    ...
    .then(data => {
      if (data.success) {
        ...
        // Update new Bootstrap card layout elements
        const vizTotalAssets = document.getElementById('vizTotalAssets');
        const vizActiveAssetsCount = document.getElementById('vizActiveAssetsCount');
        const vizRentedAssetsCount = document.getElementById('vizRentedAssetsCount');
        const vizOccupancyRate = document.getElementById('vizOccupancyRate');
        const vizOccupancyProgress = document.getElementById('vizOccupancyProgress');
        const vizMonthlyRevenue = document.getElementById('vizMonthlyRevenue');
        const vizRevenueGrowth = document.getElementById('vizRevenueGrowth');
        const vizAvgRentalPrice = document.getElementById('vizAvgRentalPrice');
        const vizPriceRange = document.getElementById('vizPriceRange');
        const vizActiveRenters = document.getElementById('vizActiveRenters');
        const vizNewRentersThisMonth = document.getElementById('vizNewRentersThisMonth');
        const vizAvgRentalDuration = document.getElementById('vizAvgRentalDuration');
        const vizRetentionRate = document.getElementById('vizRetentionRate');
        const vizPendingRequests = document.getElementById('vizPendingRequests');
        const vizApprovedRequests = document.getElementById('vizApprovedRequests');
        const vizExpiringContracts = document.getElementById('vizExpiringContracts');
        const vizRenewalRate = document.getElementById('vizRenewalRate');
        
        // BACKWARD COMPATIBILITY: Support old layout elements
        const mainTotalAssets = document.getElementById('mainTotalAssets');
        const mainPendingRequests = document.getElementById('mainPendingRequests');
        const mainMonthlyRevenue = document.getElementById('mainMonthlyRevenue');
        const mainTotalUsers = document.getElementById('mainTotalUsers');
        const mainRentedAssets = document.getElementById('mainRentedAssets');
        const mainOccupancyRate = document.getElementById('mainOccupancyRate');
        
        // Update Total Assets (New Layout) - Use real data or 0
        if (vizTotalAssets) {
          vizTotalAssets.innerHTML = stats.total_assets || 0;
        }
        
        if (vizActiveAssetsCount) {
          const activeAssets = stats.available_assets || 0;
          vizActiveAssetsCount.innerHTML = `${activeAssets} Aktif`;
          vizActiveAssetsCount.style.display = activeAssets > 0 ? 'inline' : 'none';
        }
        
        if (vizRentedAssetsCount) {
          const rentedAssets = stats.rented_assets || 0;
          vizRentedAssetsCount.innerHTML = `${rentedAssets} Tersewa`;
          vizRentedAssetsCount.style.display = rentedAssets > 0 ? 'inline' : 'none';
        }
        
        // Update Total Assets (Old Layout - Backward Compatibility)
        if (mainTotalAssets) {
          const totalAssets = stats.total_assets || 0;
          mainTotalAssets.innerHTML = `<span style="color: #28a745; font-weight: bold;">${totalAssets}</span>`;
        }
        
        // Update Occupancy Rate (New Layout) - Use real data or 0
        if (vizOccupancyRate) {
          const rate = stats.occupancy_rate || 0;
          vizOccupancyRate.innerHTML = `${rate}%`;
        }
        
        if (vizOccupancyProgress) {
          const rate = stats.occupancy_rate || 0;
          vizOccupancyProgress.style.width = `${rate}%`;
          vizOccupancyProgress.setAttribute('aria-valuenow', rate);
        }
        
        // Update Occupancy Rate (Old Layout - Backward Compatibility)
        if (mainOccupancyRate) {
          const rate = stats.occupancy_rate || 0;
          mainOccupancyRate.innerHTML = `${rate}%`;
        }
        
        // Update Monthly Revenue (New Layout) - Use real data or 0
        if (vizMonthlyRevenue) {
          const revenue = stats.monthly_revenue || 0;
          vizMonthlyRevenue.innerHTML = revenue > 0 ? `Rp ${revenue.toLocaleString('id-ID')}` : 'Rp 0';
        }
        
        if (vizRevenueGrowth) {
          const growth = stats.revenue_growth || 0;
          vizRevenueGrowth.innerHTML = `+${growth}% MoM`;
          vizRevenueGrowth.style.display = growth > 0 ? 'inline' : 'none';
        }
        
        // Update Monthly Revenue (Old Layout - Backward Compatibility)
        if (mainMonthlyRevenue) {
          const revenue = stats.monthly_revenue || 0;
          mainMonthlyRevenue.innerHTML = revenue > 0 ? `Rp ${revenue.toLocaleString('id-ID')}` : 'Rp 0';
        }
        
        // Update Average Rental Price (New Layout)
        if (vizAvgRentalPrice) {
          const avgPrice = stats.avg_rental_price || 0;
          vizAvgRentalPrice.innerHTML = avgPrice > 0 ? `Rp ${Math.round(avgPrice).toLocaleString('id-ID')}` : 'Rp 0';
        }
        ...
        if (vizPriceRange) {
          ...
          if (minPrice > 0 && maxPrice > 0) {
            vizPriceRange.innerHTML = `Rp ${minPrice.toLocaleString('id-ID')} - Rp ${maxPrice.toLocaleString('id-ID')}`;
            vizPriceRange.style.display = 'inline';
          } else {
            vizPriceRange.style.display = 'none';
          }
        }
        
        // Update Active Renters (New Layout)
        if (vizActiveRenters) {
          const activeRenters = stats.active_renters || 0;
          vizActiveRenters.innerHTML = activeRenters;
        }
        
        if (vizNewRentersThisMonth) {
          const newRenters = stats.new_renters_this_month || 0;
          vizNewRentersThisMonth.innerHTML = `+${newRenters} Bulan Ini`;
          vizNewRentersThisMonth.style.display = newRenters > 0 ? 'inline' : 'none';
        }
        
        // Update Average Rental Duration (New Layout)
        if (vizAvgRentalDuration) {
          const duration = stats.avg_rental_duration || 0;
          const formattedDuration = Number.isInteger(duration) ? duration : Math.round(duration);
          vizAvgRentalDuration.innerHTML = formattedDuration > 0 ? `<strong>${formattedDuration} bulan</strong>` : '<strong>0 bulan</strong>';
        }
        
        if (vizRetentionRate) {
          const retention = stats.retention_rate || 0;
          vizRetentionRate.innerHTML = `${Math.round(retention)}% Retensi`;
          vizRetentionRate.style.display = retention > 0 ? 'inline' : 'none';
        }
        
        // Update Pending Requests (New Layout) - Use real data or 0
        if (vizPendingRequests) {
          vizPendingRequests.innerHTML = stats.pending_requests || 0;
        }
        
        if (vizApprovedRequests) {
          const approved = stats.approved_requests || 0;
          vizApprovedRequests.innerHTML = `${approved} Disetujui`;
          vizApprovedRequests.style.display = approved > 0 ? 'inline' : 'none';
        }
        
        // Update Pending Requests (Old Layout - Backward Compatibility)
        if (mainPendingRequests) {
          mainPendingRequests.innerHTML = stats.pending_requests || 0;
        }
        
        // Update Expiring Contracts
        if (vizExpiringContracts) {
          const expiring = stats.expiring_contracts || 0;
          vizExpiringContracts.innerHTML = `<strong>${expiring}</strong>`;
        }
        
        if (vizRenewalRate) {
          const renewal = stats.renewal_rate || 0;
          vizRenewalRate.innerHTML = `${Math.round(renewal)}% Renewal`;
          vizRenewalRate.style.display = renewal > 0 ? 'inline' : 'none';
        }
        
        // Update Users (Old Layout - Backward Compatibility)
        if (mainTotalUsers) {
          mainTotalUsers.innerHTML = stats.total_users || 0;
        }
        
        // Update Rented Assets (Old Layout - Backward Compatibility)
        if (mainRentedAssets) {
          mainRentedAssets.innerHTML = stats.rented_assets || 0;
        }
        
        // Update badge for pending requests in rental requests tab
        ...
        if (pendingRequestsBadge) {
          ...
          pendingRequestsBadge.innerHTML = pendingCount;
          pendingRequestsBadge.style.display = pendingCount > 0 ? 'inline' : 'none';
        }
        
        // Update rental request counts in the rental requests section
        ...
        if (pendingRequestsCount) {
          pendingRequestsCount.innerHTML = stats.pending_requests || 0;
        }
        if (approvedRequestsCount) {
          approvedRequestsCount.innerHTML = stats.approved_requests || 0;
        }
        if (rejectedRequestsCount) {
          // Calculate rejected requests if we don't have it directly
          const totalRequests = (stats.pending_requests || 0) + (stats.approved_requests || 0);
          rejectedRequestsCount.innerHTML = 0; // Default to 0 for now
        }
        if (totalRequestsCount) {
          const totalRequests = (stats.pending_requests || 0) + (stats.approved_requests || 0);
          totalRequestsCount.innerHTML = totalRequests;
        }
        
        // Update API status indicator to show success
        const apiStatusIndicator = document.getElementById('apiStatusIndicator');
        if (apiStatusIndicator) {
          apiStatusIndicator.innerHTML = '<i class="fas fa-check-circle me-1"></i>Data API Terbaru';
          apiStatusIndicator.className = 'badge bg-success';
        }
        ...
        if (lastUpdated) {
          const now = new Date();
          lastUpdated.innerHTML = `Terakhir diperbarui: ${now.toLocaleString('id-ID')}`;
        }
        
        console.log("Dashboard statistics loaded successfully");
      } else {
        console.error("Failed to load dashboard statistics:", data.message);
        
        // Update API status indicator to show error
        const apiStatusIndicator = document.getElementById('apiStatusIndicator');
        if (apiStatusIndicator) {
          apiStatusIndicator.innerHTML = '<i class="fas fa-exclamation-triangle me-1"></i>Error API';
          apiStatusIndicator.className = 'badge bg-danger';
        }
    ...
    .catch(error => {
      ...
      const apiStatusIndicator = document.getElementById('apiStatusIndicator');
      if (apiStatusIndicator) {
        apiStatusIndicator.innerHTML = '<i class="fas fa-exclamation-triangle me-1"></i>Koneksi Error';
        apiStatusIndicator.className = 'badge bg-danger';
      }
...
function showLoadingStates() {
  // Update API status indicator to show loading
  const apiStatusIndicator = document.getElementById('apiStatusIndicator');
  if (apiStatusIndicator) {
    apiStatusIndicator.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Memuat Data API...';
    apiStatusIndicator.className = 'badge bg-warning';
  }
  
  // Elements that show loading spinners
  const loadingElements = [
    'vizTotalAssets', 'vizOccupancyRate', 'vizMonthlyRevenue', 'vizAvgRentalPrice',
    'vizActiveRenters', 'vizAvgRentalDuration', 'vizPendingRequests', 'vizExpiringContracts'
  ];
  ...
  // Hide badges during loading
  const badgeElements = [
    'vizActiveAssetsCount', 'vizRentedAssetsCount', 'vizRevenueGrowth', 'vizPriceRange',
    'vizNewRentersThisMonth', 'vizRetentionRate', 'vizApprovedRequests', 'vizRenewalRate'
  ];
...
function showErrorStates() {
  // Set error state for new layout
  const newElements = [
    'vizTotalAssets', 'vizOccupancyRate', 'vizMonthlyRevenue', 'vizAvgRentalPrice',
    'vizActiveRenters', 'vizAvgRentalDuration', 'vizPendingRequests', 'vizExpiringContracts'
  ];
  
  // Set error state for old layout (backward compatibility)
  const oldElements = [
    'mainTotalAssets', 'mainPendingRequests', 'mainMonthlyRevenue',
    'mainTotalUsers', 'mainRentedAssets', 'mainOccupancyRate'
  ];
  
  [...newElements, ...oldElements].forEach(id => {
    const element = document.getElementById(id);
    if (element) {
...
// Function to load rental requests dynamically
function loadRentalRequests() {
  ...
  fetch('/api/rental-requests')
    ...
    .then(data => {
      if (data.success) {
        const requestsList = document.getElementById('rentalRequestsList');
          ...
          requestsList.innerHTML = `
            <tr>
              <td colspan="9" class="text-center py-4">
                ...
                Belum ada permintaan sewa<br>
                <small class="text-muted">Permintaan sewa dari pengguna akan muncul di sini</small>
              </td>
            </tr>
          `;
        } else {
            ...
            <tr id="request-${request.id}" data-request-id="${request.id}">
              ...
              <td>
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    <strong>${request.user_name || request.nama_penyewa || 'Unknown User'}</strong><br>
                    <small class="text-muted">${request.user_email || request.email || 'No email'}</small>
                  </div>
                </div>
              </td>
              <td>
                <strong>${request.asset_name || 'Unknown Asset'}</strong><br>
                <small class="text-muted">${request.asset_location || 'Unknown Location'}</small>
              </td>
              <td><span class="badge ${request.asset_type === 'tanah' ? 'bg-success' : 'bg-info'}">${request.asset_type === 'tanah' ? 'Tanah' : 'Bangunan + Tanah'}</span></td>
              <td>${request.total_months || request.durasi_sewa || 0} Bulan</td>
              <td>${request.start_date ? new Date(request.start_date).toLocaleDateString('id-ID') : '-'}</td>
              <td><strong>Rp ${(request.total_price || 0).toLocaleString('id-ID')}</strong></td>
              <td><span class="badge ${getStatusBadgeClass(request.status)}">${getStatusText(request.status)}</span></td>
              <td>
                <div class="btn-group" role="group">
                  <button type="button" class="btn btn-sm btn-info" onclick="viewRequestDetails('${request.id}')" title="Detail">
        ...
        console.log("Rental requests loaded successfully");
      } else {
        console.error("Failed to load rental requests:", data.message);
        showRentalRequestsError();
      }
    })
    .catch(error => {
...
function updateRequestCounts(requests) {
  ...
  const approvedCountElement = document.getElementById('approvedRequestsCount');
  const rejectedCountElement = document.getElementById('rejectedRequestsCount');
  const totalCountElement = document.getElementById('totalRequestsCount');
...
function showRentalRequestsError() {
  const requestsList = document.getElementById('rentalRequestsList');
  if (requestsList) {
    requestsList.innerHTML = `
      <tr>
        <td colspan="9" class="text-center py-4">
          <i class="fas fa-exclamation-triangle text-danger mb-2" style="font-size: 2rem;"></i><br>
          <span class="text-danger">Error memuat permintaan sewa</span><br>
          <small class="text-muted">Silakan refresh halaman atau coba lagi nanti</small>
        </td>
      </tr>
    `;
  }
...
function viewRequestDetails(requestId) {
  // This function would show detailed information about the request
