...
def submit_rental():
    ...
    try:
        ...
        rental_request.nama_penyewa = data.get('nama_penyewa', user_name)
        rental_request.email = data.get('email', user_email)
        rental_request.telepon = data.get('telepon', '08123456789')  # Default phone if not provided
...
@main.route('/test-user-assets')
def test_user_assets():
    """Test route for user assets loading"""
...
@main.route('/test-rental-detail-page')
def test_rental_detail_page():
    """Test page for rental detail functionality"""
    return render_template('test_rental_detail.html')

@main.route('/debug-lihat-detail')
def debug_lihat_detail():
    """Debug page for testing Lihat Detail button"""
    return render_template('debug_lihat_detail.html')

@main.route('/api/rental-payment/<int:request_id>', methods=['POST'])
def process_rental_payment(request_id):
    """Process payment for approved rental request"""
    try:
        ...
        # Get rental request
        from app.models_sqlalchemy import RentalRequest
        rental_request = RentalRequest.query.filter_by(
            id=request_id, 
            user_id=user_id,
            status='approved'
        ).first()
        ...
        if not rental_request:
            return jsonify({'success': False, 'error': 'Rental request not found or not approved'}), 404
        ...
        order_id = f"RENTAL-{request_id}-{datetime.now().strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8]}"
        
        # Get total amount
        total_amount = int(rental_request.total_harga or rental_request.total_price or 0)
        ...
        transaction_data = {
            ...
            "customer_details": {
                "first_name": rental_request.user_name or rental_request.nama_penyewa,
                "email": rental_request.user_email or rental_request.email,
                "phone": rental_request.user_phone or rental_request.telepon or '081234567890'
            },
            "item_details": [{
                "id": f"rental-{request_id}",
                "price": total_amount,
                "quantity": 1,
                "name": f"Sewa {rental_request.asset.name if rental_request.asset else 'Aset'}",
                "category": "Rental"
            }]
        }
        
        print(f"Creating rental payment for request {request_id}: {transaction_data}")
        ...
        # Store payment info (optional - you can create a payment tracking table)
        # For now, we'll just return the snap token
        ...
        print(f"Error processing rental payment: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Payment error: {str(e)}'
        }), 500

@main.route('/api/rental-payment-callback', methods=['POST'])
def rental_payment_callback():
    ...
    try:
        ...
        # Extract rental request ID from order_id (format: RENTAL-{request_id}-...)
        if order_id and order_id.startswith('RENTAL-'):
            parts = order_id.split('-')
            if len(parts) >= 2:
                try:
                    request_id = int(parts[1])
                    
                    # Update rental request status based on payment status
                    from app.models_sqlalchemy import RentalRequest
                    rental_request = RentalRequest.query.get(request_id)
                    
                    if rental_request:
                        if transaction_status in ['capture', 'settlement']:
                            # Payment successful - update to active
                            rental_request.status = 'active'
                            rental_request.updated_at = datetime.now()
                            db.session.commit()
                            
                            print(f"Rental request {request_id} updated to active")
                            
                        elif transaction_status in ['deny', 'cancel', 'expire']:
                            # Payment failed - keep as approved for retry
                            print(f"Payment failed for rental request {request_id}: {transaction_status}")
                            
                except ValueError:
                    print(f"Invalid request_id in order_id: {order_id}")
...
@main.route('/test-bayar-sekarang')
def test_bayar_sekarang():
    """Test page for Bayar Sekarang button"""
    return render_template('test_bayar_sekarang.html')
