...
def approve_rental_request(request_id):
    ...
    try:
        ...
        print(f"Debug: Before update - Request {request_id} status: {rental_request.status}")
        rental_request.status = 'approved'
        rental_request.admin_notes = admin_notes
        rental_request.updated_at = datetime.utcnow()
        print(f"Debug: After update - Request {request_id} status: {rental_request.status}")
        
        # Ubah status aset menjadi rented
        asset = RentalAsset.query.get(rental_request.asset_id)
        if asset:
            print(f"Debug: Before asset update - Asset {asset.id} status: {asset.status}")
            asset.status = 'rented'
            print(f"Debug: After asset update - Asset {asset.id} status: {asset.status}")
