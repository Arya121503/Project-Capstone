# Consolidated Rental Assets Routes
...
def get_all_assets():
    ...
    try:
        ...
        query = query.filter(RentalAsset.status != 'deleted')
...
def delete_asset(asset_id):
    ...
    try:
        ...
        # Check if asset has transaction history
        from app.models_rental_transaction import RentalTransaction
        transaction_count = RentalTransaction.query.filter_by(asset_id=asset_id).count()
        
        if transaction_count > 0:
            # Asset has transaction history - prevent deletion to maintain data integrity
            return jsonify({
                'success': False,
                'error': f'Cannot delete asset with transaction history ({transaction_count} transactions found). This preserves rental records for legal and business purposes.',
                'transaction_count': transaction_count
            }), 400
        else:
            # No transaction history - safe to delete
