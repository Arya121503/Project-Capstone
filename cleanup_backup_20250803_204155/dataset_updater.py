...
try:
    from .simple_model_system import simple_model_system
except ImportError:
    simple_model_system = None
...
class DatasetUpdater:
    ...
    def save_models(self, models, label_encoders, scaler, feature_names, dataset_type, model_scores=None):
        ...
        if model_scores and 'ensemble' in models:
            ...
            print(f"🏆 Selected ENSEMBLE as best model for {dataset_type}")
            ...
            print(f"🏆 Selected {best_model_name.upper()} as best model for {dataset_type} (R² = {model_scores[best_model_name]['r2']:.4f})")
            ...
            best_model = models[best_model_name]
            best_model_path = os.path.join(self.models_path, f"best_{dataset_type}_model_{best_model_name}.pkl")
            print(f"⚠️ Using {best_model_name} as fallback best model for {dataset_type}")
        
        # Save the best model with standardized naming
        if best_model is not None:
            with open(best_model_path, 'wb') as f:
                pickle.dump(best_model, f)
            print(f"🎯 Saved BEST {dataset_type} model: {os.path.basename(best_model_path)}")
            ...
            with open(generic_best_path, 'wb') as f:
                pickle.dump(best_model, f)
            print(f"🔗 Created generic best model: best_{dataset_type}_model.pkl")
            
            print(f"💾 All {dataset_type} models saved successfully!")
    ...
    def _reload_predictor_models(self):
        ...
        try:
            ...
            if 'app.ml_predictor' in sys.modules:
                ...
                print("🔄 Reloaded ML predictor with new models")
    ...
    def _reload_predictor_models_enhanced(self):
        ...
        try:
            ...
            print(f"🔄 Reloaded predictor with models: {list(new_predictor.models.keys())}")
            ...
            try:
                ...
                if hasattr(current_app, 'predictor'):
                    ...
                    print("✅ Updated app-level predictor instance")
            except:
                pass
                
            print("✅ Enhanced model reload completed successfully")
            return True
            
        except Exception as e:
            print(f"⚠️ Enhanced reload failed: {e}")
            return False
    ...
    def update_dataset(self, file_path, dataset_type, backup_old_model=True):
        ...
        try:
            ...
            # Update model characteristics cache for dataset-aware predictions
            try:
                import hashlib
                ...
                # Load existing cache
                cache_file = os.path.join(self.models_path, 'model_cache.json')
                cache = {}
                ...
                # Update cache for this dataset type
                if dataset_type == 'tanah':
                    mean_njop = float(df['NJOP_Rp_per_m2'].mean()) if 'NJOP_Rp_per_m2' in df.columns else 2500000
                    mean_area = float(df['luas_tanah_m2'].mean()) if 'luas_tanah_m2' in df.columns else 1500
                    mean_rent = float(df['Sewa_per_Bulan'].mean()) if 'Sewa_per_Bulan' in df.columns else 15000000
                    ...
                    mean_njop = float(df['NJOP (Rp/m²)'].mean()) if 'NJOP (Rp/m²)' in df.columns else 3500000
                    mean_building_area = float(df['Luas Bangunan (m²)'].mean()) if 'Luas Bangunan (m²)' in df.columns else 100
                    mean_rooms = float(df['Kamar Tidur'].mean()) if 'Kamar Tidur' in df.columns else 3
                    mean_rent = float(df['Sewa per Bulan (Rp)'].mean()) if 'Sewa per Bulan (Rp)' in df.columns else 12000000
                ...
                print(f"✅ Updated dataset cache for {dataset_type}")
                print(f"   Price multiplier: {price_multiplier:.3f}")
                print(f"   Records: {len(df)}")
                print(f"   Hash: {dataset_hash}")
                
            except Exception as e:
                print(f"⚠️ Could not update dataset cache: {e}")
            ...
            # Update simple model system characteristics
            try:
                if simple_model_system is not None:
                    simple_model_system.update_model_characteristics(dataset_type, df)
                    print(f"✅ Updated simple model characteristics for {dataset_type}")
                else:
                    print("⚠️ Simple model system not available")
            except Exception as e:
                print(f"⚠️ Could not update simple model characteristics: {e}")
