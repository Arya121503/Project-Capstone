...
class RentalAssetManager {
    ...
    bindEvents() {
        ...
        searchInputs.forEach(input => {
            input.addEventListener('input', (e) => {
                ...
                this.debounce(() => this.loadAvailableAssets(), 500)();
            });
        });
    ...
    async loadAvailableAssets(page = 1) {
        try {
            ...
            const response = await fetch(`/rental/api/assets/available?${params}`);
            ...
            this.showError('#availableAssetsList', 'Failed to load available assets');
        }
    }
    ...
    displayAvailableAssets(assets) {
        ...
        tbody.innerHTML = assets.map(asset => `
            <tr>
                ...
                <td><span class="badge bg-primary">${asset.status_display || 'Tersedia'}</span></td>                    <td>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <table class="table table-sm">
                        <tr><td><strong>Nama:</strong></td><td>${this.escapeHtml(asset.name)}</td></tr>
                        <tr><td><strong>Jenis:</strong></td><td>${asset.asset_type === 'tanah' ? 'Tanah' : 'Bangunan + Tanah'}</td></tr>
                        <tr><td><strong>Lokasi:</strong></td><td>${this.escapeHtml(asset.kecamatan)}, Surabaya</td></tr>
                        <tr><td><strong>Alamat:</strong></td><td>${this.escapeHtml(asset.alamat)}</td></tr>
                        <tr><td><strong>Luas Tanah:</strong></td><td>${asset.luas_tanah} m²</td></tr>
                        ${asset.luas_bangunan ? `<tr><td><strong>Luas Bangunan:</strong></td><td>${asset.luas_bangunan} m²</td></tr>` : ''}
                    </table>
                </div>
                <div class="col-md-6">
                    <h6>Detail Properti</h6>
                    <table class="table table-sm">
                        ${asset.kamar_tidur ? `<tr><td><strong>Kamar Tidur:</strong></td><td>${asset.kamar_tidur}</td></tr>` : ''}
                        ${asset.kamar_mandi ? `<tr><td><strong>Kamar Mandi:</strong></td><td>${asset.kamar_mandi}</td></tr>` : ''}
                        ${asset.jumlah_lantai ? `<tr><td><strong>Jumlah Lantai:</strong></td><td>${asset.jumlah_lantai}</td></tr>` : ''}
                        <tr><td><strong>Sertifikat:</strong></td><td>${this.escapeHtml(asset.sertifikat)}</td></tr>
                        <tr><td><strong>Zona:</strong></td><td>${this.escapeHtml(asset.jenis_zona)}</td></tr>
                        <tr><td><strong>Harga Sewa:</strong></td><td>Rp ${this.formatCurrency(asset.harga_sewa)}/bulan</td></tr>
                        <tr><td><strong>Status:</strong></td><td><span class="badge bg-primary">${asset.status_display || 'Tersedia'}</span></td></tr>
                    </table>
                </div>
            </div>
            ${asset.deskripsi ? `
                    ...
                    <div class="col-md-12">
                        ...
                        <table class="table table-sm table-borderless">
                            ...
                            <tr><td class="fw-semibold">Status:</td><td>
                                <span class="badge bg-success">${rental.asset.status_display || 'Tersedia'}</span>
                            </td></tr>
                        </table>
    ...
    updateAssetCount(total, type) {
        const countElement = type === 'available' ? 
            document.getElementById('assetCount') :
            document.getElementById('rentedAssetCount');
        
        if (countElement) {
            countElement.textContent = total;
        }

        const totalElement = type === 'available' ? 
            document.getElementById('totalAssetCount') :
            document.getElementById('totalRentedAssetCount');
